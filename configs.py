...
def get_config(args):
    # print('DEBUG: running get_config()')
    if len(args) > 2:
        # print('DEBUG: conditional 1: taking the if')
        print('Too many arguments. Only one argument allowed.')
        get_config(args)
    elif len(args) < 2:
        # print('DEBUG: conditional 1: taking the elif')
        print('Not enough arguments. Please enter the name or nickname of a Cubase config.')
        get_config(args)
    else:
        # print('DEBUG: conditional 1: taking the else')
        checked_configs = []
        ...
        for config in user_configs:
            ...
            # if specified_config == config['user'] or specified_config == config['nickname']:
                # print('DEBUG: conditional 2: taking the if')
                # print(f'DEBUG: {config['path'}')
                # breakpoint()
                return config['path']
            else:
                # print('DEBUG: conditional 2: taking the else')
