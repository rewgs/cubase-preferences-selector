...
def main():
    ...
    default_items = []
    for item in Path.iterdir(configs.default_config):
        default_items.append(item.name)

    user_items = []
    for item in Path.iterdir(user_config):
    ...
    for item in to_swap:
        if item in user_items:
            user_item = Path(
                PurePath.joinpath(user_config, item)
            ).resolve()

            if item in default_items:
                default_item = Path(
                    PurePath.joinpath(configs.default_config, item)
                )
            ...
            # this branch runs if the default Cubase config is incomplete for some reason
                ...
                default_item = PurePath.joinpath(configs.default_config, item)
                if user_item.is_dir():
